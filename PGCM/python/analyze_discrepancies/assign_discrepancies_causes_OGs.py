...
def query_in_psl(psl_df, query, min_frac=0.9):
  """
...
def assign_cause(row, transcripts_vs_assembly_dict, prot_vs_sample_proteins_dict, variant_annotation_dict, og_orig_dict, og_mwop_dict, min_frac):
  """
  ...
  ## DN-/MTP+ discrepancies
    ...
    # if transcript found in assembly
      ...
      if not psl_hit.empty:
        # if no match between DN and MTP gene in the discrep. table
        if pd.isnull(row.iloc[1]):
          cause = 'pan-gene_matching_issue'
        # if a match exists, check for clustering issues
        else:
          target = psl_hit['tName']
          if are_clustered_differently(row.iloc[1], target, og_orig_dict, og_mwop_dict):
            cause = 'clustering_issue'
...
if __name__ == "__main__":
  ...
  parser.add_argument('og_tsv', help='Orthogrougs TSV as created by OrthoFinder')
  parser.add_argument('og_break_tsv', help='Orthogroups TSV after MWOP')
  ...
  # Read orthogroups - before and after MWOP
  og_orig_dict = parse_og_tsv(args.og_tsv)
  og_mwop_dict = parse_og_tsv(args.og_break_tsv)

  # Assign causes
  apply_args = [transcripts_vs_assembly_dict, prot_vs_sample_proteins_dict, variant_annotation_dict, og_orig_dict, og_mwop_dict, args.min_frac]
